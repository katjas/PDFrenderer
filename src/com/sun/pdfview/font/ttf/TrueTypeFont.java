...
public class TrueTypeFont {
	...
	private static void parseDirectories(ByteBuffer data, int numTables, TrueTypeFont ttf) {
		for (int i = 0; i < numTables; i++) {
			...
			// read the data
			PDFDebugger.debug(
					"TTFFont.parseDirectories: checksum: " + checksum + ", offset: " + offset + ", length: " + length,
					100);
			data.mark();
			...
			if (calcChecksum == checksum) {
				...
				PDFDebugger.debug(
						"Mismatched checksums on table " + tagString + ": " + calcChecksum + " != " + checksum, 200);
	...
	public TrueTypeFont(int type) {
		...
		this.tables = Collections.synchronizedSortedMap(new TreeMap<String, Object>());
	}
