...
/**
 * A representation, with parser, of an Adobe Type 1C font. You can find
 * information about CFF and Type 1C font encoding in
 * http://partners.adobe.com/public/developer/en/font/5176.CFF.pdf and
...
public class Type1CFont extends OutlineFont {
	...
	private int glyphnames[];

	private int encoding[] = new int[256];

	private AffineTransform at = new AffineTransform(0.001f, 0, 0, 0.001f, 0, 0);
	...
	/**
	 * calculate an offset code for a dictionary. Uses the count of entries to
	...
	/**
	 * get the index of a particular name. The name table starts with the
	 * standard names in FontSupport.stdNames, and is appended by any names in
	 * the name table from this font's dictionary.
	 */
	 ...
	 * @return the name from the FontSupport.stdNames table augmented by the
	 *         local name table
	 */
	...
	private void parse() throws IOException {
		...
		new String(this.data, r.getStart(), r.getLen());
		// read first dict
	...
	public void parseGlyph(Range r, GeneralPath gp, FlPoint pt) {
		...
		while (this.pos < r.getEnd()) {
			...
			case 3: // vstem
				...
				break;
			case 4: // vmoveto
				if (this.stackptr > 1) { // this is the first call, arg1 is
											// width
			...
			case 14: // endchar
				...
				if (this.stackptr == 5) {
					buildAccentChar(this.stack[1], this.stack[2], (char) this.stack[3], (char) this.stack[4], gp);
				} else if (this.stackptr == 4) {
					// see page 58 on specification 5177.Type2.pdf which
					// indicates that
					// these parameters are valid for Type1C as the width is
					// optional
	...
	/**
	 * read a complete command. this may involve several numbers which go onto a
	 * stack before an actual command is read.
	 * 
	 * @param charstring
	 *            ????
	...
	private void readDict(Range r) {
		...
		while (this.pos < r.getEnd()) {
			...
			if (cmd == 1006) { // charstringtype, default=2
				...
				if (this.stackptr == 4) {
					this.at = new AffineTransform(this.stack[0], this.stack[1], this.stack[2], this.stack[3], 0, 0);
				} else {
					this.at = new AffineTransform(this.stack[0], this.stack[1], this.stack[2], this.stack[3],
	...
	/**
	 * Read the data for a glyph from the glyph table, and transform it based on
	...
	private int readNext(boolean charstring) {
		...
		if (this.num == 30 && !charstring) { // goofy floatingpoint rep
			...
			this.num = (this.data[this.pos] & 0xff) << 24 | (this.data[this.pos + 1] & 0xff) << 16
					| (this.data[this.pos + 2] & 0xff) << 8 | this.data[this.pos + 3] & 0xff;
			this.pos += 4;
			return NUM;
			...
			this.fnum = ((this.data[this.pos] & 0xff) << 24 | (this.data[this.pos + 1] & 0xff) << 16
					| (this.data[this.pos + 2] & 0xff) << 8 | this.data[this.pos + 3] & 0xff) / 65536f;
			this.pos += 4;
			return FLT;
		}
	}
	...
	/**
	 * convert a string to one in which any non-printable bytes are replaced by
	 * "<###>" where ## is the value of the byte.
	 */
