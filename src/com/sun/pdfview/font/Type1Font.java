...
public class Type1Font extends OutlineFont {
	...
	class PSParser {
		...
		/**
		 * treat the next n bytes of the input stream as encoded information to
		 * be decrypted.
		 * 
		 * @param n
		 ...
		 *            decryption
		 * @return an array of decrypted bytes. The length of the array will be
		 *         n-skip.
		 */
		...
		/**
		 * read a set of numbers from the input. This method doesn't pay any
		 * attention to "[" or "]" delimiters, and reads any non-numeric items
		 * as the number 0.
		 * 
		 * @param count
		 ...
		 * non-whitespace block between two whitespace delimiters. This means
		 * that something like " [2 4 53]" will produce three items, while " [2
		 * 4 56 ]" will produce four.
		 */
	 ...
	 * 
	 * @param x
	 *            the x offset of the accent relativ to the sidebearing of the
	 *            base char
	 * @param y
	 *            the y offset of the accent relativ to the sidebearing of the
	 *            base char
	 * @param a
	...
	private void buildAccentChar(float x, float y, char a, char b, GeneralPath gp) {
		...
		try {
			...
			// Best x can't be calculated cause we don't know the left
			// sidebearing of the base character.
			// Leaving x=0 gives the best results.
			// see Chapter 6 of
			// http://partners.adobe.com/public/developer/en/font/5015.Type1_Supp.pdf
			// and the definition of the seac-Command in
	...
	protected GeneralPath getOutline(String name, float width) {
		...
		// get whatever is stored in name. Could be a GeneralPath, could be
		// byte[]
	...
	/**
	 * Determine if data is in ASCII or binary format. According to the spec, if
	 * any of the first 4 bytes are not character codes ('0' - '9' or 'A' - 'F'
	 * or 'a' - 'f'), then the data is binary. Otherwise it is ASCII
	 */
	...
	/**
	 * parse glyph data into a GeneralPath, and return the advance width. The
	 * working point is passed in as a parameter in order to allow recursion.
	 * 
	 * @param cs
	...
	private void parse(byte[] cs, GeneralPath gp, FlPoint pt, FlPoint wid) {
		...
		while (loc < cs.length) {
			int v = cs[loc++] & 0xff;
			if (v == 255) {
				this.stack[this.sloc++] = ((cs[loc] & 0xff) << 24) + ((cs[loc + 1] & 0xff) << 16)
						+ ((cs[loc + 2] & 0xff) << 8) + (cs[loc + 3] & 0xff);
				loc += 4;
			} else if (v >= 251) {
				...
				case 8: // x1 y1 x2 y2 x3 y3 rcurveto
					...
					y1 = pop();
					x1 = pop();
					gp.curveTo(pt.x + x1, pt.y + y1, pt.x + x1 + x2, pt.y + y1 + y2, pt.x + x1 + x2 + x3,
							pt.y + y1 + y2 + y3);
					pt.x += x1 + x2 + x3;
					pt.y += y1 + y2 + y3;
					this.sloc = 0;
				...
				case 10: // n callsubr
					...
					if (n == 0) {
						if (flexPt != 14) {
							...
							gp.curveTo(flexArray[2], flexArray[3], flexArray[4], flexArray[5], flexArray[6],
									flexArray[7]);
							gp.curveTo(flexArray[8], flexArray[9], flexArray[10], flexArray[11], flexArray[12],
									flexArray[13]);
							flexMode = false;
							this.sloc = 0;
	 ...
	 * <p>
	 * this function attempts to parse an array out of a postscript definition
	 * without doing any postscript. It's actually looking for things that look
	 * like "dup <i>id</i> <i>elt</i> put", and placing the <i>elt</i> at the
	 * <i>i</i>th position in the array.
	 * 
	 * @param d
	 ...
	 * @param d
	 *            the font data
	 * @return a HashMap that maps string glyph names to byte arrays of decoded
	 *         font data.
	 */
