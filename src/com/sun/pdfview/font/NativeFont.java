...
public class NativeFont extends OutlineFont {
	...
	protected static final short[] mapIDs = { 3, 1, /* Microsoft Unicode */
			0, 0, /* unicode default */
			0, 3, /* unicode 2.0 map */
			1, 0 /* macintosh */ };
	...
	private final FontRenderContext basecontext = new FontRenderContext(new AffineTransform(), true, true);
	...
	/**
	 * create a new NativeFont object based on a description of the font from
	 * the PDF file. If the description happens to contain an in-line true-type
	 * font file (under key "FontFile2"), use the true type font. Otherwise,
	 * parse the description for key information and use that to generate an
	 * appropriate font.
	 */
	 ...
	 * control characters (0x9, 0xa, 0xd) Java will not render them, even though
	 * they are valid.
	 *
	 * Also, Java tends to not like it when there is only a Format 0 CMap, which
	 * happens frequently when included Format 4 CMaps are broken. Since PDF
	 * prefers the Format 0 map, while Java prefers the Format 4 map, it is
	 * generally necessary to re-write the Format 0 map as a Format 4 map to
	 * make most PDFs work.
	 *
	 * @param ttf
	 *            the font
	...
	/**
	 * Fix a broken font name table for a TrueType font. Some fonts do not have
	 * Microsoft-specific name information, but Java won't work without it
	 * (grrr.). This method takes a font and adds the Microsoft data into it.
	 *
	 * @param ttf
	 *            the font
	 * @param name
	...
	private boolean fixNameTable(TrueTypeFont ttf, NameTable name) {
		...
		short[] nameIDs = { NameTable.NAMEID_COPYRIGHT, NameTable.NAMEID_FAMILY, NameTable.NAMEID_SUBFAMILY,
				NameTable.NAMEID_SUBFAMILY_UNIQUE, NameTable.NAMEID_FULL_NAME, NameTable.NAMEID_VERSION,
				NameTable.NAMEID_POSTSCRIPT_NAME, NameTable.NAMEID_TRADEMARK };

		String[] defaultValues = { "No copyright", fName, style, fName + " " + style, fName + " " + style, "1.0 (Fake)",
				fName, "No Trademark" };
	...
	protected void setFont(byte[] fontdata) throws FontFormatException, IOException {
		try {
			...
			/*
			 * Find out if we have the right info in our name table. This is a
			 * hack because Java can only deal with fonts that have a Microsoft
			 * encoded name in their name table (PlatformID 3). We'll 'adjust'
			 * the font to add it if not, and take our chances with our parsing,
			 * since it wasn't going to work anyway.
			 */
			...
			/*
			 * Figure out if we need to hack the CMap table. This might be the
			 * case if we use characters that Java considers control characters
			 * (0x9, 0xa and 0xd), that have to be re-mapped
			 */
