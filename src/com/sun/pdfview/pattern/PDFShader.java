 ...
 * including the type of shading to be used, the geometry of the area to be
 * shaded, and the geometry of the gradient fill. Various shading types are
 * available, depending on the value of the dictionary’s ShadingType entry:
 * 
 ...
 * boundary colors.</li>
 * 
 * <li>Radial shadings (type 3) define a blend between two circles, optionally
 * extended beyond the boundary circles by continuing the boundary colors. This
 * type of shading is commonly used to represent three-dimensional spheres and
 * cones.</li>
 * 
 * <li>Free-form Gouraud-shaded triangle meshes (type 4) define a common
 * construct used by many three-dimensional applications to represent complex
 * colored and shaded shapes. Vertices are specified in free-form geometry.</li>
 * 
 * <li>Lattice-form Gouraud-shaded triangle meshes (type 5) are based on the
 * same geometrical construct as type 4 but with vertices specified as a
 * pseudorectangular lattice.</li>
 * 
 * <li>Coons patch meshes (type 6) construct a shading from one or more color
 * patches, each bounded by four cubic Bézier curves.</li>
 * 
 * <li>Tensor-product patch meshes (type 7) are similar to type 6 but with
 * additional control points in each patch, affording greater control over color
 * mapping.</li>
 * 
...
public abstract class PDFShader {
	...
	public static PDFShader getShader(PDFObject shaderObj, Map resources) throws IOException {
		...
		if (bboxObj != null) {
			...
			Rectangle2D bbox = new Rectangle2D.Float(minX, minY, maxX - minX, maxY - minY);
			shader.setBBox(bbox);
		}
