...
public class PatternType1 extends PDFPattern {
	...
	static class TilingPatternPaint extends PDFPaint {
		 ...
		 *            the path to fill
		 * @param drawn
		 *            a Rectangle2D into which the dirty area (area drawn) will
		 *            be added.
		 */
		@Override
	...
	/**
	 * A simple paint context that uses an existing raster in device space to
	 * generate pixels
	 */
	...
	/**
	 * Create a PDFPaint from this pattern and set of components. This creates a
	 * buffered image of this pattern using the given paint, then uses that
	 * image to create the correct TexturePaint to use in the PDFPaint.
	 *
	 * @param basePaint
	...
	public PDFPaint getPaint(PDFPaint basePaint) {
		...
		/*
		 * AffineTransform xform = new AffineTransform(1, 0, 0, -1, 0,
		 * getYStep()); //new AffineTransform(1, 0, 0, -1, 0,
		 * getBBox().getHeight()); page.addXform(xform);
		 */
	...
	protected void parse(PDFObject patternObj, Map rsrc) throws IOException {
		...
		this.bbox = new Rectangle2D.Float(bboxObj.getAt(0).getFloatValue(), bboxObj.getAt(1).getFloatValue(),
				bboxObj.getAt(2).getFloatValue(), bboxObj.getAt(3).getFloatValue());
