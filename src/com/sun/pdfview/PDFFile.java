 ...
 * you create a new PDFFile, ask it for the number of pages, and then request
 * one or more PDFPages.
 * 
 * @author Mike Wessler
 */
public class PDFFile {
	 ...
	 *             unsupported. If the file is encrypted in a manner that the
	 *             product or platform does not support then the exception's
	 *             {@link PDFParseException#getCause() cause} will be an
	 *             instance of {@link UnsupportedEncryptionException}.
	 ...
	 *             instance of {@link UnsupportedEncryptionException}.
	 * @throws PDFAuthenticationFailureException
	 *             if the file is password protected and the supplied password
	...
	/**
	 * Gets whether the owner of the file has given permission to print the
	 * file.
	 * 
	...
	/**
	 * Gets whether the owner of the file has given permission to save a copy of
	 * the file.
	 * 
	...
	/**
	 * return the number of pages in this PDFFile. The pages will be numbered
	 * from 1 to getNumPages(), inclusive.
	 */
	...
	/**
	 * Get metadata (e.g., Author, Title, Creator) from the Info dictionary as a
	 * string.
	 * 
	 * @param name
	...
	/**
	 * Used internally to track down PDFObject references. You should never need
	 * to call this.
	 * <p>
	 * Since this is the only public method for tracking down PDF objects, it is
	 * synchronized. This means that the PDFFile can only hunt down one object
	 * at a time, preventing the file's location from getting messed around.
	 * <p>
	...
	/**
	 * Is the argument a white space character according to the PDF spec?. ISO
	 ...
	 * 
	 * @param objNum
	 *            the object number of the object containing the object being
	 *            read; negative only if the object number is unavailable (e.g.,
	 *            if reading from the trailer, or reading at the top level, in
	 *            which case we can expect to be reading an object description)
	 * @param objGen
	 ...
	 * 
	 * @param numscan
	 *            if true, don't bother trying to see if a number is an object
	 *            reference (used when already in the middle of testing for an
	 *            object reference, and not otherwise)
	 * @param objNum
	 *            the object number of the object containing the object being
	 *            read; negative only if the object number is unavailable (e.g.,
	 *            if reading from the trailer, or reading at the top level, in
			...
			if (c == '<') {
				...
				if (!numscan) {
					...
					if (testnum != null && testnum.getType() == PDFObject.NUMBER) {
						...
						if (testR != null && testR.getType() == PDFObject.KEYWORD
								&& "R".equals(testR.getStringValue())) {
	 ...
	 * 
	 * @param match
	 *            the next few characters after any whitespace that must be in
	 *            the file
	...
	/**
	 * process a version string, to determine the major and minor versions of
	 * the file.
	 * 
	 * @param versionString
	 */
	...
	/**
	 * read an entire &lt;&lt; dictionary &gt;&gt;. The initial &lt;&lt; has
	 * already been read.
	 * 
	 * @param objNum
	 *            the object number of the object containing the dictionary
	 *            being read; negative only if the object number is unavailable,
	 *            which should only happen if we're reading a dictionary placed
	 *            directly in the trailer
	 * @param objGen
	 *            the object generation of the object containing the object
	 *            being read; negative only if the objNum is unavailable
	 * @param decrypter
	...
	/**
	 * return the 8-bit value represented by the next two hex characters. If the
	 * next two characters don't represent a hex value, return -1 and reset the
	 * read head. If there is only one hex character, return its value as if
	 * there were an implicit 0 after it.
	 */
	 ...
	 *            the object number of the object containing the dictionary
	 *            being read; negative only if the object number is unavailable,
	 *            which should only happen if we're reading a string placed
	 *            directly in the trailer
	 * @param objGen
	 *            the object generation of the object containing the object
	 *            being read; negative only if the objNum is unavailable
	...
	/**
	 * <p>
	 * read a ( character string ). The initial ( has already been read. Read
	 * until a *balanced* ) appears.
	 * </p>
	 *
	 * <p>
	 * Section 3.2.3 of PDF Refernce version 1.7 defines the format of String
	 * objects. Regarding literal strings:
	 * </p>
	 *
	 ...
	 * characters, nonprinting ASCII characters, unbalanced parentheses, or the
	 * backslash character itself in the string. The character immediately
	 * following the backslash determines its precise interpretation (see Table
	 * 3.2). If the character following the backslash is not one of those shown
	 * in the table, the backslash is ignored.</blockquote>
	 *
	 * *
	 * <p>
	 * This only reads 8 bit basic character 'strings' so as to avoid a text
	 * string interpretation when one is not desired (e.g., for byte strings, as
	 * used by the decryption mechanism). For an interpretation of a string
	 * returned from this method, where the object type is defined as a 'text
	 * string' as per Section 3.8.1, Table 3.31 "PDF Data Types",
	 ...
	 * If a '\r' is encountered, it is discarded.
	 */
	...
	/**
	 * read an [ array ]. The initial [ has already been read. PDFObjects are
	 * read until ].
	 * 
	 * @param objNum
	 *            the object number of the object containing the dictionary
	 *            being read; negative only if the object number is unavailable,
	 *            which should only happen if we're reading an array placed
	 *            directly in the trailer
	 * @param objGen
	 *            the object generation of the object containing the object
	 *            being read; negative only if the objNum is unavailable
	...
	/**
	 * read an entire PDFObject. The intro line, which looks something like "4 0
	 * obj" has already been read.
	 * 
	 * @param objNum
	 *            the object number of the object being read, being the first
	 *            number in the intro line (4 in "4 0 obj")
	 * @param objGen
	 *            the object generation of the object being read, being the
	 *            second number in the intro line (0 in "4 0 obj").
	 * @param decrypter
	 *            the decrypter to use
	 */
	private PDFObject readObjectDescription(int objNum, int objGen, PDFDecrypter decrypter) throws IOException {
		...
		if (endkey.getType() != PDFObject.KEYWORD && endkey.getType() != PDFObject.STREAM) {
			PDFDebugger.debug("WARNING: Expected 'stream' or 'endobj' but was " + endkey.getType() + " "
	...
	/**
	 * read the stream portion of a PDFObject. Calls decodeStream to un-filter
	 * the stream as necessary.
	 *
	 * @param dict
	...
	/**
	 * read the cross reference table from a PDF file. When this method is
	 * called, the file pointer must point to the start of the word "xref" in
	 * the file. Reads the xref table and the trailer dictionary. If dictionary
	 * has a /Prev entry, move file pointer and read new trailer
	 * 
	 * @param password
	 */
	private void readTrailer(PDFPassword password) throws IOException, PDFAuthenticationFailureException,
		...
		while (true) {
			...
			if (!nextItemIs("xref")) {
				...
				return;
				// throw new PDFParseException("Expected 'xref' at start of
				// table");
			}
			...
			while (true) {
				...
				if (refstart == 1) {// Check and try to fix incorrect Object
									// Number Start
					int startPos = this.buf.position();
					try {
						...
						if (refline[17] == 'f') {// free
							...
							if (objIndex.getID() == 0 && objIndex.getGeneration() == 65535) { // The
																								// highest
																								// generation
																								// number
																								// possible
			...
			if (this.encrypt == null) {
				...
				if (this.encrypt != null && !PDFDecrypterFactory.isFilterExist(this.encrypt)) {
					this.encrypt = null; // the filter is not located at this
											// trailer, we will try later again
				} else {
					newDefaultDecrypter = PDFDecrypterFactory.createDecryptor(this.encrypt,
							trailerdict.getDictRef("ID"), password);
				}
			}
			...
			// support for hybrid-PDFs containing an additional
	...
	private void readTrailer15(PDFPassword password) throws IOException, PDFAuthenticationFailureException,
		...
		// the table of xrefs
		// objIdx is initialized from readTrailer(), do not overwrite here data
		// from hybrid PDFs
		...
		while (true) {
			...
			if (this.encrypt == null) {
				...
				if (this.encrypt != null && !PDFDecrypterFactory.isFilterExist(this.encrypt)) {
					...
					newDefaultDecrypter = PDFDecrypterFactory.createDecryptor(this.encrypt, trailerdict.get("ID"),
							password);
				}
			}
	...
	/**
	 * build the PDFFile reference table. Nothing in the PDFFile actually gets
	 * parsed, despite the name of this function. Things only get read and
	 * parsed when they're needed.
	 * 
	 * @param password
	 */
	...
	/**
	 * Gets the outline tree as a tree of OutlineNode, which is a subclass of
	 * DefaultMutableTreeNode. If there is no outline tree, this method returns
	 * null.
	 */
	...
	/**
	 * Gets the page number (starting from 1) of the page represented by a
	 * particular PDFObject. The PDFObject must be a Page dictionary or a
	 * destination description (or an action).
	 * 
	 * @return a number between 1 and the number of pages indicating the page
	 *         number, or 0 if the PDFObject is not in the page tree.
	 */
	 ...
	 * @return a concatenation of any content streams for the requested page.
	 */
		...
		if (annots != null) {
			...
			for (PDFObject object : array) {
				try {
					...
					// do nothing, annotations could not be parsed and links
	 ...
	 * that the number of the page need not have anything to do with the label
	 * on that page. If there are two blank pages, and then roman numerals for
	 * the page number, then passing in 6 will get page (iv).
	 *
	 * @param pagedict
	 ...
	 * @param resources
	 *            a HashMap that will be filled with any resource definitions
	 *            encountered on the search for the page
	 */
	private PDFObject findPage(PDFObject pagedict, int start, int getPage, Map<String, PDFObject> resources)
			throws IOException {
	...
	/**
	 * Find a property value in a page that may be inherited. If the value is
	 * not defined in the page itself, follow the page's "parent" links until
	 ...
	 * 
	 * @return the default decrypter; never null, even for documents that aren't
	 *         encrypted
	 */
