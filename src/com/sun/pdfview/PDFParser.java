...
/**
 * PDFParser is the class that parses a PDF content stream and produces PDFCmds
 * for a PDFPage. You should never ever see it run: it gets created by a PDFPage
 * only if needed, and may even run in its own thread.
 *
 * @author Mike Wessler
 */
public class PDFParser extends BaseWatchable {
	 ...
	 * pagenum) to get a PDFPage. There should never be any reason for a user to
	 * create, access, or hold on to a PDFParser.
	 */
	...
	/**
	 * Inject a stream of PDF commands onto the page. Optimized to cache a
	 * parsed stream of commands, so that each Form object only needs to be
	 * parsed once.
	 *
	 * @param obj
	 *            a stream containing the PDF commands, a transformation matrix,
	...
	private void doForm(PDFObject obj) throws IOException {
		...
		if (formCmds == null) {
			...
			bbox = new Rectangle2D.Float(bobj.getAt(0).getFloatValue(), bobj.getAt(1).getFloatValue(),
					bobj.getAt(2).getFloatValue(), bobj.getAt(3).getFloatValue());
	...
	/**
	 * Parse image data into a Java BufferedImage and add the image command to
	 * the page.
	 *
	 * @param obj
	 *            contains the image data, and a dictionary describing the
	 *            width, height and color space of the image.
	 */
	...
	/**
	 * Insert a PDF object into the command stream. The object must either be an
	 * Image or a Form, which is a set of PDF commands in a stream.
	 *
	 * @param obj
	...
	// ///////////////////////////////////////////////////////////////
	/**
	 * get a property from a named dictionary in the resources of this content
	 * stream.
	 *
	 * @param name
	...
	/**
	 * parse the stream. commands are added to the PDFPage initialized in the
	 * constructor as they are encountered.
	 * <p>
	 ...
	 *         <li>Watchable.COMPLETED when the page is done and all the
	 *         commands have been processed
	 *         <li>Watchable.STOPPED if the page we are rendering into is no
	 *         longer available
	 *         </ul>
	 */
	...
	public int iterate() throws Exception {
		...
		if (obj instanceof Tok) {
			...
			if ("q".equals(cmd)) {
				...
				this.path = new GeneralPath();
				PDFDebugger.logPath(path, "new path");
			} else if ("f".equals(cmd) || "F".equals(cmd)) {
				tryClosingPath();
				...
				if (!PDFDebugger.DISABLE_PATH_STROKE_FILL
						|| !PDFDebugger.DISABLE_CLIP && this.clip == PDFShapeCmd.CLIP) {
				...
				// if (this.state.strokeCS instanceof PatternSpace) {
				// this.cmds.addFillPaint(doPattern((PatternSpace)
				// this.state.strokeCS));
				// } else {
				// int n = this.state.strokeCS.getNumComponents();
				// this.cmds.addStrokePaint(this.state.strokeCS.getPaint(popFloat(n)));
			...
			if (this.stack.size() != 0) {
				PDFDebugger.debug("**** WARNING! Stack not zero! (cmd=" + cmd + ", size=" + this.stack.size()
						+ ") *************************", 10);
	...
	/**
	 * generate a PDFColorSpace description based on a PDFObject. The object
	 * could be a standard name, or the name of a resource in the ColorSpace
	 * dictionary, or a color space name with a defining dictionary or stream.
	 */
	...
	/**
	 * Parse an inline image. An inline image starts with BI (already read,
	...
	private void parseInlineImage() throws IOException, DebugStopException {
		...
		if (imObj != null && imObj.getBooleanValue()) {
			// [PATCHED by michal.busta@gmail.com] - default value according to
			// PDF spec. is [0, 1]
			// there is no need to swap array - PDF image should handle this
			// values
		...
		while (!PDFFile.isWhiteSpace(this.stream[this.loc]) || this.stream[this.loc + 1] != 'E'
	...
	/**
	 * Parse the next object out of the PDF stream. This could be a Double, a
	 * String, a HashMap (dictionary), Object[] array, or a Tok containing a PDF
	 * command.
	 */
	private Object parseObject() throws PDFParseException, DebugStopException {
		...
		PDFDebugger.debug("**** WARNING! parseObject unknown token! (t.type=" + t.type + ") *************************",
				10);
		return null;
	}
	...
	/**
	 * pop an array of float values off the stack. This is equivalent to filling
	 * an array from end to front by popping values off the stack.
	 *
	 * @param count
	...
	/**
	 * pop an array of integer values off the stack. This is equivalent to
	 * filling an array from end to front by popping values off the stack.
	 *
	 * @param count
	 *            the number of numbers to pop off the stack
	 * @return an array of length <tt>count</tt>
	...
	/**
	 * abstracted command processing for BT command. Used directly and as part
	 * of processing of mushed QBT command.
	 */
	...
	/**
	 * abstracted command processing for Q command. Used directly and as part of
	 * processing of mushed QBT command.
	 */
	 ...
	 * character, which has already been read, and end with a '>' character.
	 * Each byte in the array is made up of two hex characters, the first being
	 * the high-order bit.
	 *
	 * We translate the byte arrays into char arrays by combining two bytes into
	 * a character, and then translate the character array into a string. [JK
	 * FIXME this is probably a really bad idea!]
	 *
	...
	/**
	 * <p>
	 * read a String from the stream. Strings begin with a '(' character, which
	 * has already been read, and end with a balanced ')' character. A '\'
	 * character starts an escape sequence of up to three octal digits.
	 * </p>
	 *
	 * <p>
	 * Parenthesis must be enclosed by a balanced set of parenthesis, so a
	 * string may enclose balanced parenthesis.
	 * </p>
	 *
	...
	private void setGSState(String name) throws IOException {
		...
		// TODO: lots of graphic states are not yet considered, see chapter
		// 8.4.5 of the PDF specification.
