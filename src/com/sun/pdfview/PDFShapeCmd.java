...
/**
 * Encapsulates a path. Also contains extra fields and logic to check for
 * consecutive abutting anti-aliased regions. We stroke the shared line between
 * these regions again with a 1-pixel wide line so that the background doesn't
...
public class PDFShapeCmd extends PDFCmd {
	...
	private BasicStroke againstroke = new BasicStroke(2, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL);
	private boolean autoAdjustStroke = false;

	/**
	 * create a new PDFShapeCmd and check it against the previous one to find
	 * any shared edges.
	 *
	 * @param gp
	 *            the path
	 * @param style
	 *            the style: an OR of STROKE, FILL, or CLIP. As a convenience,
	 *            BOTH = STROKE | FILL.
	 */
				...
				for (int i = 0; i < prevcount; i += 4) {
					for (int j = 0; j < mycount; j += 4) {
						if (Math.abs(mypoints[j + 2] - prevpoints[i]) < 0.01
							...
							// it seems that need to use the original points
							// location (without the Affine Transform)
							mypoints = new float[16];
							getPoints(this.gp, mypoints, null);// without
																// AffineTransform
