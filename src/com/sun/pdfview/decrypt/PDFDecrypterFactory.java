...
public class PDFDecrypterFactory {
	...
	/**
	 * Create a decrypter working from a crypt filter dictionary, as in version
	 * 4 encryption
	 *
	 * @param encryptDict
	 ...
	 *             if the provided password does not decrypt this document
	 * @throws IOException
	 *             if there is a problem reading the PDF, an invalid document
	 *             structure, or an inability to obtain the required ciphers
	 ...
	 *             encryption
	 */
	private static PDFDecrypter createCryptFilterDecrypter(PDFObject encryptDict, PDFObject documentId,
			PDFPassword password, int v) throws PDFAuthenticationFailureException, IOException,
	 ...
	 *
	 * @param encryptDict
	 *            the Encrypt dict as found in the document's trailer. May be
	 *            null, in which case the {@link IdentityDecrypter} will be
	 *            returned.
	 * @param documentId
	 *            the object with key "ID" in the trailer's dictionary. Should
	 *            always be present if Encrypt is.
	 * @param password
	 ...
	 * @throws IOException
	 *             will typically be a
	 *             {@link com.sun.pdfview.PDFParseException}, indicating an IO
	 *             problem, an error in the structure of the document, or a
	 *             failure to obtain various ciphers from the installed JCE
	 *             providers
	 * @throws EncryptionUnsupportedByPlatformException
	 *             if the encryption is not supported by the environment in
	 *             which the code is executing
	...
	public static PDFDecrypter createDecryptor(PDFObject encryptDict, PDFObject documentId, PDFPassword password)
			throws IOException, EncryptionUnsupportedByPlatformException, EncryptionUnsupportedByProductException,
		...
		if (encryptDict == null) {
			...
			if (filter != null && "Standard".equals(filter.getStringValue())) {
				...
				if (v == 1 || v == 2) {
					...
					return createStandardDecrypter(encryptDict, documentId, password, length, false,
							StandardDecrypter.EncryptionAlgorithm.RC4);
				} else if (v == 4) {
	 ...
	 *            the password
	 * @param keyLength
	 *            the key length, in bits; may be <code>null</code> to use a
	 *            {@link #DEFAULT_KEY_LENGTH default}
	 ...
	 * @throws PDFAuthenticationFailureException
	 *             if the provided password is not the one expressed by the
	 *             encryption dictionary
	 * @throws IOException
	 *             if there is a problem reading the PDF content, if the content
	 *             does not comply with the PDF specification
	 * @throws EncryptionUnsupportedByPlatformException
	 *             if the encryption is not supported by the environment in
	 *             which the code is executing
	 * @throws EncryptionUnsupportedByProductException
	 ...
	 *
	 */
	private static PDFDecrypter createStandardDecrypter(PDFObject encryptDict, PDFObject documentId,
			PDFPassword password, Integer keyLength, boolean encryptMetadata,
			StandardDecrypter.EncryptionAlgorithm encryptionAlgorithm) throws PDFAuthenticationFailureException,
		...
		if (revision < 2 || revision > 4) {
			throw new EncryptionUnsupportedByPlatformException(
					"Unsupported Standard security handler revision; R=" + revision);
		}
		...
		return new StandardDecrypter(encryptionAlgorithm, documentId, keyLength, revision, o, u, pObj.getIntValue(),
