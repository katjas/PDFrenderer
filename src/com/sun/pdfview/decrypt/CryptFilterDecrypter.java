 ...
 * contains a list of named 'crypt filters', each of which is the equivalent of
 * a {@link PDFDecrypter}. In addition to this list of crypt filters, the name
 * of the filter to use for streams and the default filter to use for strings is
 * specified. Requests to decode a stream with a named decrypter (typically
 * Identity) instead of the default decrypter are honoured.
 *
 * @author Luke Kirby
 */
public class CryptFilterDecrypter implements PDFDecrypter {
	 ...
	 * 
	 * @param decrypters
	 *            a map of crypt filter names to their corresponding decrypters.
	 *            Must already contain the Identity filter.
	 * @param defaultStreamCryptName
	 *            the crypt filter name of the default stream decrypter
	 * @param defaultStringCryptName
	 *            the crypt filter name of the default string decrypter
	 * @throws PDFParseException
	 *             if one of the named defaults is not present in decrypters
	 */
	public CryptFilterDecrypter(Map<String, PDFDecrypter> decrypters, String defaultStreamCryptName,
			String defaultStringCryptName) throws PDFParseException {
		...
		if (defaultStreamDecrypter == null) {
			throw new PDFParseException(
					"Unknown crypt filter specified as default for streams: " + defaultStreamCryptName);
		}
		defaultStringDecrypter = this.decrypters.get(defaultStringCryptName);
		if (defaultStringDecrypter == null) {
			throw new PDFParseException(
					"Unknown crypt filter specified as default for strings: " + defaultStringCryptName);
		}
	}
