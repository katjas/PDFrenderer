...
public class StandardDecrypter implements PDFDecrypter {
	 ...
	 */
	private final static byte[] PW_PADDING = new byte[] { 0x28, (byte) 0xBF, 0x4E, 0x5E, 0x4E, 0x75, (byte) 0x8A, 0x41,
			0x64, 0x00, 0x4E, 0x56, (byte) 0xFF, (byte) 0xFA, 0x01, 0x08, 0x2E, 0x2E, 0x00, (byte) 0xB6, (byte) 0xD0,
			0x68, 0x3E, (byte) 0x80, 0x2F, 0x0C, (byte) 0xA9, (byte) 0xFE, 0x64, 0x53, 0x69, 0x7A };
	 ...
	 *             encryption
	 */
	public StandardDecrypter(EncryptionAlgorithm encryptionAlgorithm, PDFObject documentId, int keyBitLength,
			int revision, byte[] oValue, byte[] uValue, int pValue, boolean encryptMetadata, PDFPassword password)
			throws IOException, EncryptionUnsupportedByProductException, EncryptionUnsupportedByPlatformException {
		...
		try {
			...
			for (int i = 0; this.generalKeyBytes == null && i < passwordBytePossibilities.size(); ++i) {
				...
				this.generalKeyBytes = checkOwnerPassword(passwordBytes, firstDocIdValue, keyBitLength, revision,
						oValue, uValue, pValue, encryptMetadata);
				if (this.generalKeyBytes != null) {
					...
					this.generalKeyBytes = checkUserPassword(passwordBytes, firstDocIdValue, keyBitLength, revision,
							oValue, uValue, pValue, encryptMetadata);
	 ...
	 *             ciphers/digests
	 */
	private byte[] calculateGeneralEncryptionKey(byte[] userPassword, byte[] firstDocIdValue, int keyBitLength,
			int revision, byte[] oValue, int pValue, boolean encryptMetadata) throws GeneralSecurityException {
		...
		// Step 5: Pass the first element of the fileâs file identifier
		// array
		...
		// 3 or greater, depends on the value of the encryption
		// dictionaryâs
		// Length entry.
	...
	private byte[] calculateUValue(byte[] generalKey, byte[] firstDocIdValue, int revision)
			throws GeneralSecurityException, EncryptionUnsupportedByProductException {
	 ...
	 */
	private byte[] checkOwnerPassword(byte[] ownerPassword, byte[] firstDocIdValue, int keyBitLength, int revision,
			byte[] oValue, byte[] uValue, int pValue, boolean encryptMetadata)
			throws GeneralSecurityException, EncryptionUnsupportedByProductException, PDFParseException {
		...
		if (revision == 2) {
			...
			// dictionaryâs O entry, using an RC4 encryption function with
			// the
			...
			// value of the encryption dictionaryâs O entry (first
			// iteration) or
		...
		return checkUserPassword(possibleUserPassword, firstDocIdValue, keyBitLength, revision, oValue, uValue, pValue,
				encryptMetadata);
	 ...
	 */
	private byte[] checkUserPassword(byte[] userPassword, byte[] firstDocIdValue, int keyBitLength, int revision,
			byte[] oValue, byte[] uValue, int pValue, boolean encryptMetadata)
			throws GeneralSecurityException, EncryptionUnsupportedByProductException, PDFParseException {
		...
		final byte[] generalKey = calculateGeneralEncryptionKey(userPassword, firstDocIdValue, keyBitLength, revision,
				oValue, pValue, encryptMetadata);
		...
		// encryption dictionaryâs U entry (comparing on the first 16 bytes
		// in
	 ...
	 */
	private Cipher createAndInitialiseContentCipher(ByteBuffer encrypted, byte[] decryptionKeyBytes)
			throws PDFParseException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException,
	...
	private void testJceAvailability(int keyBitLength)
			throws EncryptionUnsupportedByPlatformException, PDFParseException {
		...
		try {
			...
			throw new EncryptionUnsupportedByPlatformException("JCE does accept key size of "
					+ getSaltedContentKeyByteLength() * 8 + " bits- could it be a policy restriction?", e);
		...
		if (this.encryptionAlgorithm != EncryptionAlgorithm.RC4) {
			...
			try {
				...
				throw new EncryptionUnsupportedByPlatformException(
						"JCE did not accept 40-bit RC4 key; " + "policy problem?", ex);
			}
		}
	}
