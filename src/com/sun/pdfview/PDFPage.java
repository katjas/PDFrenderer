...
class PDFChangeStrokeCmd extends PDFCmd {
	...
	public String toString(PDFRenderer state) {
		return "STROKE: w=" + this.w + " cap=" + this.cap + " join=" + this.join + " limit=" + this.limit + " ary="
				+ this.ary + " phase=" + this.phase;
	}
}
...
public class PDFPage {
	...
	public PDFPage(int pageNumber, Rectangle2D bbox, int rotation, Cache cache) {
		...
		rotation = rotation / 90; // for page rotation use only multiples of 90
									// degrees
	...
	/**
	 * add a collection of commands to the page list. This is probably invoked
	 * as the result of an XObject 'do' command, or through a type 3 font.
	 */
	 ...
	 * @param extra
	 *            a transform to perform before adding the commands. If null, no
	 *            extra transform will be added.
	 */
	...
	/**
	 * Get an image producer which can be used to draw the image represented by
	 * this PDFPage. The ImageProducer is guaranteed to stay in sync with the
	 * PDFPage as commands are added to it.
	 *
	 ...
	 * @param observer
	 *            an image observer who will be notified when the image changes,
	 *            or null
	 ...
	 *            or null
	 * @param drawbg
	 *            if true, put a white background on the image. If not, draw no
	 *            color (alpha 0) for the background.
	 * @param wait
	 *            if true, do not return until this image is fully rendered.
	 * @return an Image that contains the PDF data
	 */
	public Image getImage(int width, int height, Rectangle2D clip, ImageObserver observer, boolean drawbg,
			boolean wait) {
	...
	/**
	 * Get the initial transform to map from a specified clip rectangle in pdf
	 * coordinates to an image of the specfied width and height in device
	 * coordinates
	 *
	 * @param width
	 ...
	 * @param clip
	 *            the desired clip rectangle (in PDF space) or null to use the
	 *            page's bounding box
	 */
	 ...
	 * @param clip
	 *            the region in <b>page space</b> of the page to display. It may
	 *            be null, in which the page's defined crop box will be used.
	 */
	...
	/**
	 * get whether parsing for this PDFPage has been completed and all commands
	 * are in place.
	 */
