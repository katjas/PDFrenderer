...
/**
 * An abstract implementation of the watchable interface, that is extended by
...
public abstract class BaseWatchable implements Watchable, Runnable {
	 ...
	 * tell us to stop after that much time or that many steps
	 */
	static class Gate {
		...
		/**
		 * Notify the gate of one iteration. Returns true if we should stop or
		 * false if not
		 */
	 ...
	 * stopped. Note the watchable may be stopped if go() with a different time
	...
	/**
	 * Start this watchable and run until it is finished or stopped. Note the
	 * watchable may be stopped if go() with a different time is called during
	 * execution.
	 *
	 * @param synchronous
	 *            if true, run in this thread
	...
	/**
	 * Return whether this watchable has finished. A watchable is finished when
	 * its status is either COMPLETED, STOPPED or ERROR
	 */
						...
						try {
							...
							while (getStatus() == Watchable.RUNNING && (this.gate == null || !this.gate.iterate())) {
								...
								if (status != laststatus) {
									// update status only when necessary, this
	...
	/**
	 * Stop this watchable if it is not already finished. Stop will cause all
	 * processing to cease, and the watchable to be destroyed.
	 */
	@Override
	public void stop() {
		if (!isFinished()) {
