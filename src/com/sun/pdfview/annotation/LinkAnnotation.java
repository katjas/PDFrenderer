 ...
 * document, a location in another PDF file, an application/file to be opened or
 * a web site. In the PDF structure a link can be a destination ("DEST") or an
 * action ("A"). Both ways are handled as actions internally, i.e. for getting
 * the links destination, you should get the action from this annotation object.
 * It can be one of the following actions:
...
public class LinkAnnotation extends PDFAnnotation {
	...
	public LinkAnnotation(PDFObject annotObject) throws IOException {
		...
		// a link annotation can either have an action (GoTo or URI) or a
		// destination (DEST)
		...
		if (actionObj != null) {
			...
			if (dest != null) {
				...
				throw new PDFParseException(
						"Could not parse link annotation (no Action or Destination found): " + annotObject.toString());
			}
		}
	}
	 ...
	 * 
	 * @return PDFAction - can be <code>null</code> in case the contains a
	 *         destination object
	 ************************************************************************/
