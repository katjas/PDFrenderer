...
public class CMYKColorSpace extends ColorSpace {
	...
	public CMYKColorSpace() {
		...
		try {
			icc = ICC_Profile.getInstance(
					getClass().getResourceAsStream("/ch/randelshofer/media/jpeg/Generic_CMYK_Profile.icc"));
	...
	/**
	 * Converts to CMYK from CIEXYZ. We cheat here, using the RGB colorspace to
	 * do the math for us. The toCIEXYZ function has a description of how this
	 * is supposed to work, which may be implemented in the future.
	 * 
	 ...
	 * always be 0. On printers, the black should actually look dark brown. RGB
	 * (an additive space) is simply the backwards from CMY (a subtractive
	 ...
	 * cheat, converting to RGB and then using the RGB colorspace function to do
	 * the conversion. Details on this colorspace are available on wikipedia:
	 * 
	 ...
	 * color.org):
	 * 
	 * C = (C' - K)/(1 - K) M = (M' - K)/(1 - K) Y = (Y' - K)/(1 - K) K =
	 * Min(C', M', Y')
	 * 
	 * X 41.2453 35.7580 18.0423 | 1-C' Y = 21.2671 71.5160 07.2169 | 1-M' Z
	 * 01.9334 11.9193 95.0227 | 1-Y'
	 * 
	 ...
	 * formula for this is:
	 * 
	 * K = 1 - K (go to additive) R = K * (1 - C) G = K * (1 - M) B = K * (1 -
	 * Y)
	 * 
	 * @param p_colorvalue
	 *            The color in CMYK.
