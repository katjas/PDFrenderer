...
/**
 * a class encapsulating all the possibilities of content for an object in a PDF
 * file.
 * <p>
 * A PDF object can be a simple type, like a Boolean, a Number, a String, or the
 * Null value. It can also be a NAME, which looks like a string, but is a
 * special type in PDF files, like "/Name".
 * <p>
 * A PDF object can also be complex types, including Array; Dictionary; Stream,
 * which is a Dictionary plus an array of bytes; or Indirect, which is a
 * reference to some other PDF object. Indirect references will always be
 * dereferenced by the time any data is returned from one of the methods in this
 * class.
 *
 * @author Mike Wessler
 */
public class PDFObject {
	...
	/**
	 * When a value of {@link #getObjGen objNum} or {@link #getObjGen objGen},
	 * indicates that the object is not top-level, and is embedded directly in
	 * the trailer.
	 */
	...
	/**
	 * a cache of translated data. This data can be garbage collected at any
	 * time, after which it will have to be rebuilt.
	 */
	 ...
	 * Possibilities include Double, String, PDFObject[], HashMap, Boolean, or
	 * PDFParser.Tok, which should be "true" or "false" to turn into a BOOLEAN.
	 *
	 * @param obj
	 *            the sample Java object to convert to a PDFObject.
	 * @throws PDFParseException
	 *             if the object isn't one of the above examples, and can't be
	 *             turned into a PDFObject.
	 */
	 ...
	 * 
	 * @param owner
	 *            the PDFFile in which this object resides, used for
	 *            dereferencing. This may be null.
	 * @param type
	 *            the type of object
	 * @param value
	 *            the value. For DICTIONARY, this is a HashMap. for ARRAY it's
	 *            an ArrayList. For NUMBER, it's a Double. for BOOLEAN, it's
	 *            Boolean.TRUE or Boolean.FALSE. For everything else, it's a
	 *            String.
	 */
	...
	/**
	 * Make sure that this object is dereferenced. Use the cache of an indirect
	 * object to cache the dereferenced value, if possible.
	 */
	...
	/**
	 * Test whether two PDFObject are equal. Objects are equal IFF they are the
	 * same reference OR they are both indirect objects with the same id and
	 * generation number in their xref
	 */
	@Override
	...
	/**
	 * get the value as a PDFObject[]. If this object is an ARRAY, will return
	 * the array. Otherwise, will return an array of one element with this
	...
	/**
	 * if this object is an ARRAY, get the PDFObject at some position in the
	 * array. If this is not an ARRAY, returns null.
	 */
	...
	/**
	 * get the value as a boolean. Will return false if this object is not a
	 * BOOLEAN
	 */
	 ...
	 * 
	 * @return the cached value, or null if the value has been garbage
	 *         collected.
	 */
	...
	/**
	 * get the dictionary as a HashMap. If this isn't a DICTIONARY or a STREAM,
	 * returns null
	 */
	...
	/**
	 * get an Iterator over all the keys in the dictionary. If this object is
	 * not a DICTIONARY or a STREAM, returns an Iterator over the empty list.
	 */
	...
	/**
	 * get the value associated with a particular key in the dictionary. If this
	 * isn't a DICTIONARY or a STREAM, or there is no such key, returns null.
	 */
	 ...
	 * embedded within another object. If the value is {@link #OBJ_NUM_TRAILER},
	 * it's because it's an object from the trailer.
	 * 
	...
	/**
	 * Get the object number of this object; a negative value indicates that the
	 * object is not numbered, as it's not a top-level object: if the value is
	 * {@link #OBJ_NUM_EMBEDDED}, it is because it's embedded within another
	 * object. If the value is {@link #OBJ_NUM_TRAILER}, it's because it's an
	...
	/**
	 * get the stream from this object. Will return null if this object isn't a
	 * STREAM.
	 * 
	...
	/**
	 * get the value as a String. Will return null if the object isn't a STRING,
	 * NAME, or KEYWORD. This method will <b>NOT</b> convert a NUMBER to a
	 * String. If the string is actually a text string (i.e., may be encoded in
	 * UTF16-BE or PdfDocEncoding), then one should use
	 * {@link #getTextStringValue()} or use one of the {@link PDFStringUtil}
	 * methods on the result from this method. The string value represents
	 * exactly the sequence of 8 bit characters present in the file, decrypted
	 * and decoded as appropriate, into a string containing only 8 bit character
	 * values - that is, each char will be between 0 and 255.
	 */
	...
	/**
	 * Get the value as a text string; i.e., a string encoded in UTF-16BE or
	 * PDFDocEncoding. Simple latin alpha-numeric characters are preserved in
	...
	/**
	 * get the type of this object. The object will be dereferenced, so INDIRECT
	...
	/**
	 * returns true only if this object is a DICTIONARY or a STREAM, and the
	 * "Type" entry in the dictionary matches a given value.
	 * 
	 * @param match
	 *            the expected value for the "Type" key in the dictionary
	...
	/**
	 * set the cached value. The object may be garbage collected if no other
	...
	/**
	 * set the stream of this object. It should have been a DICTIONARY before
	 * the call.
	 * 
	 * @param data
	...
	/**
	 * return a representation of this PDFObject as a String. Does NOT
	 * dereference anything: this is the only method that allows you to
	 * distinguish an INDIRECT PDFObject.
	 */
	@Override
	public String toString() {
		try {
			if (type == INDIRECT) {
				...
				/*
				 * } else if (type==IMAGE) { StringBuffer sb= new
				 * StringBuffer(); java.awt.Image im= (java.awt.Image)stream;
				 * sb.append("Image ("+im.getWidth(null)+"x"+im.getHeight(null)
				 * +", with keys:"); HashMap hm= (HashMap)value; Iterator it=
				 * hm.keySet().iterator(); while(it.hasNext()) {
				 * sb.append(" "+(String)it.next()); } return sb.toString();
				 */
			} else {
