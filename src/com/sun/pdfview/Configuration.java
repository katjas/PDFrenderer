...
public class Configuration {
	...
	/**
	 * threshold in pixels after which images are rendered in chunks (disabled
	 * by default)
	 */
	 ...
	 * rendering.
	 * 
	 * @return the threshold value, or a value <= 0 if banded rendering is
	 *         disabled
	 */
	...
	/**
	 * Set this to false to switch off the use of this color convert op which
	 * may segfault on some platforms due to a variety of problems related to
	 * thread safety and the native cmm library underlying this conversion op,
	 * e.g.,
	 ...
	 * 
	 * If the system is bug-free, though, this does make use of native libraries
	 * and sees a not insignificant speed-up, though it's still not exactly
	 * fast. If we don't run this op now, it's performed at some later stage,
	 * but without using the native code
	 * 
	...
	/**
	 * Enables or disables the conversion of greyscale images to ARGB. Disabling
	 * this may have a lower memory overhead with high resolution (e.g. scanned)
	 * images. Note that this has to be called before {@link #getImage()} is
	 * called to have an effect.
	 *
	 * Enabled by default.
	 * 
	 * @param aFlag
	 *            whether greyscale images shall be converted to ARGB.
	 */
	...
	/**
	 * If an image is higher than the given size (in pixels) then the image will
	 * be rendered in chunks, rather than as one big image. This may lead to
	 * lower memory consumption for e.g. scanned PDFs with large images.
	 *
	 * Set to a value <= 0 to disable banded image rendering. Defaults to 0
	 * (off)
	 *
	 * @param aSize
	 *            the height threshold at which to enable banded image rendering
	 */
