...
final class Copy implements PostScriptOperation {
	 ...
	 * topmost operand. In the first form, where the top element on the operand
	 * stack is a nonnegative integer n, copy pops n from the stack and
	 * duplicates the top n elements on the stack as shown above. This form of
	 * copy operates only on the objects themselves, not on the values of
	 * composite objects.
	 * <p>
	 *
	 * Examples<br>
	 ...
	 * objects themselves (see Section 3.3.1, "Simple and Composite Objects").
	 * However, copy performs only one level of copying. It does not apply
	 * recursively to elements that are themselves composite objects; instead,
	 * the values of those elements become shared. In the case of arrays or
	 * strings, the length of the second object must be at least as great as the
	 * first; copy returns the initial subarray or substring of the second
	 * operand into which the elements were copied. Any remaining elements of
	 * array2 or string2 are unaffected.
	 * <p>
	 * 
	 * Example: <br>
	 * /a1 [1 2 3] def<br>
