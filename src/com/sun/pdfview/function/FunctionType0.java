...
public class FunctionType0 extends PDFFunction {
	...
	protected void doFunction(float[] inputs, int inputOffset, float[] outputs, int outputOffset) {
		...
		for (int i = 0; i < getNumInputs(); i++) {
			...
			encoded[i] = interpolate(inputs[i + inputOffset], getDomain(2 * i), getDomain(2 * i + 1), getEncode(2 * i),
					getEncode(2 * i + 1));
		...
		for (int i = 0; i < outputs.length; i++) {
			...
			outputs[i + outputOffset] = interpolate(outputs[i + outputOffset], 0,
					(float) Math.pow(2, getBitsPerSample()) - 1, getDecode(2 * i), getDecode(2 * i + 1));
		}
	}
