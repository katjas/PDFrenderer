 ...
 * encapsulates the state of drawing in terms of stroke, fill, transform, etc.,
 * as well as pushing and popping these states.
 *
...
public class PDFRenderer extends BaseWatchable implements Runnable {
	...
	public class GraphicsState implements Cloneable {
		 ...
		 *
		 * Note that cliprgn is not cloned. It must be set manually from the
		 * current graphics object's clip
		 */
		@Override
		public Object clone() {
			...
			cState.stroke = new BasicStroke(this.stroke.getLineWidth(), this.stroke.getEndCap(),
					this.stroke.getLineJoin(), this.stroke.getMiterLimit(), this.stroke.getDashArray(),
	 ...
	 * @param clip
	 *            the portion of the page to draw, in page space, or null if the
	 *            whole page should be drawn
	 * @param bgColor
	 *            the color to draw the background of the image, or null for no
	 *            color (0 alpha value)
	 */
	...
	public void addObserver(ImageObserver observer) {
		...
		if (rendererFinished()) {
			...
			observer.imageUpdate(i, ImageObserver.ALLBITS, 0, 0, this.imageinfo.width, this.imageinfo.height);
			return;
		} else {
			...
			if (this.globalDirtyRegion != null) {
				observer.imageUpdate(i, ImageObserver.SOMEBITS, (int) this.globalDirtyRegion.getMinX(),
						(int) this.globalDirtyRegion.getMinY(), (int) this.globalDirtyRegion.getWidth(),
	...
	/**
	 * auto adjust the stroke width, according to 6.5.4, which presumes that the
	 * device characteristics (an image) require a single pixel wide line, even
	 * if the width is set to less. We determine the scaling to see if we would
	 * produce a line that was too small, and if so, scale it up to produce a
	 * graphics line of 1 pixel, or so. This matches our output with Adobe
	 * Reader.
	 * 
	 * @param g
	 * @param bs
	...
	private BasicStroke autoAdjustStrokeWidth(Graphics2D g, BasicStroke bs) {
		...
		if (width < 1f) {
			...
			stroke = new BasicStroke(width, bs.getEndCap(), bs.getLineJoin(), bs.getMiterLimit(), bs.getDashArray(),
					bs.getDashPhase());
		}
		return stroke;
	}
	...
	public Rectangle2D drawImage(PDFImage image) {
		AffineTransform at = new AffineTransform(1f / image.getWidth(), 0, 0, -1f / image.getHeight(), 0, 1);
		...
		if (Configuration.getInstance().isUseBlurResizingForImages() && bi.getType() != BufferedImage.TYPE_CUSTOM
		...
		// banded rendering may lead to lower memory consumption for e.g.
	...
	/**
	 * Convert an image mask into an image by painting over any pixels that have
	 * a value in the image with the current paint
	 */
		...
		if (colorModel instanceof IndexColorModel) {
			...
			if (mapSize == 2 && pixelSize == 1) {
				...
				if (rgbValues[0] == 0xff000000) {
					...
					colorComponents = new byte[] { (byte) col.getRed(), (byte) col.getGreen(), (byte) col.getBlue(),
							(byte) col.getAlpha(), 0, 0, 0, 0 // the background
																// is
																// transparent
					};
				} else if (rgbValues[1] == 0xff000000) {
					// alternate case color at 1
					colorComponents = new byte[] { 0, 0, 0, 0, // the background
																// is
																// transparent
							(byte) col.getRed(), (byte) col.getGreen(), (byte) col.getBlue(), (byte) col.getAlpha() };
				}
				...
				if (colorComponents != null) {
					...
					ColorModel replacementColorModel = new IndexColorModel(pixelSize, mapSize, colorComponents,
							startIndex, hasAlpha);
	...
	/**
	 * Draws the next command in the PDFPage to the buffered image. The image
	 * will be notified about changes no less than every UPDATE_DURATION
	 * milliseconds.
	 *
	 * @return
	 *         <ul>
	 ...
	 *         <li>Watchable.STOPPED if the image we are rendering into has gone
	 *         away
	 *         </ul>
	 */
	@Override
	...
	/**
	 * restore the state of this object to what it was when the previous push()
	 * was called.
	 */
	public void pop() {
	...
	/**
	 * push the current graphics state onto the stack. Continue working with the
	 * current object; calling pop() restores the state of this object to its
	 * state when push() was called.
	 */
	public void push() {
	...
	/**
	 * set the clip to be the given shape. The current clip is not taken into
	 * account.
	 */
	...
	public void setFillAlpha(float alpha) {
		this.state.fillAlpha = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha);
	}
	...
	public void setStrokeAlpha(float alpha) {
		this.state.strokeAlpha = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha);
	}
	 ...
	 * @param ary
	 *            the dash array, or null to leave it unchanged. phase and ary
	 *            must both be valid, or phase must be NOPHASE while ary is
	 *            null.
	 */
		...
		if (Configuration.getInstance().isUseBlurResizingForImages() && bi.getType() != BufferedImage.TYPE_CUSTOM
			...
			if (image.getWidth() > maxFactor * r.getWidth() && image.getHeight() > maxFactor * r.getHeight()) {
				...
				BufferedImage resized = new BufferedImage(newWidth, newHeight,
						colorConversion ? BufferedImage.TYPE_INT_ARGB : bi.getType());
				...
				final float[] blurKernel = { weight, weight, weight, weight, weight, weight, weight, weight, weight,
						weight, weight, weight, weight, weight, weight, weight, };
				op = new ConvolveOp(new Kernel(4, 4, blurKernel), ConvolveOp.EDGE_NO_OP, null);
			} else {
				final float weight = 1.0f / 18.0f;
				final float[] blurKernel = { 1 * weight, 2 * weight, 1 * weight, 2 * weight, 6 * weight, 2 * weight,
						1 * weight, 2 * weight, 1 * weight };
				if (colorConversion) {
					BufferedImage colored = new BufferedImage(image.getWidth(), image.getHeight(),
							colorConversion ? BufferedImage.TYPE_INT_ARGB : bi.getType());
	 ...
	 * @param s
	 *            the path to stroke
	 * @return a Rectangle2D to which the current region being drawn will be
	 *         added. May also be null, in which case no dirty region will be
	 *         recorded.
	 */
