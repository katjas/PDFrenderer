 ...
 * allows 2 pixels made of 3 4-bit-bands (p[pixel,band]) to be stored in 3 bytes
 * as p[0,1] p[0,2] | p[0,3] p[1,0] | p[1,1] p[1,2].
...
public class PdfSubByteSampleModel extends SampleModel {
	...
	public PdfSubByteSampleModel(int w, int h, int numComponents, int bitsPerComponent) {
		...
		assert bitsPerComponent == 1 || bitsPerComponent == 2
				|| bitsPerComponent == 4 : "we don't want to grab components across byte boundaries";
