 ...
 * (though without consequence if the string consists only of ASCII alphanumeric
 * values). Care must be taken to either convert basic strings to text strings
 * (also expressed as a String) when appropriate, using either the methods in
 * this class, or {@link PDFObject#getTextStringValue()}}. For strings that are
 * 'byte strings', {@link #asBytes(String)} or {@link PDFObject#getStream()}
 * should be used.
 * </p>
 * .
 *
...
public class PDFStringUtil {
	...
	/**
	 * Maps from PDFDocEncoding bytes to unicode characters. Table generated by
	 * PDFDocEncodingMapGenerator.
	 */
	public final static char[] PDF_DOC_ENCODING_MAP = new char[] { 0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005,
			0x0006, 0x0007, // 00-07
	...
	/**
	 * Create a basic string from bytes. This is effectively the byte array cast
	 * to a char array and turned into a String.
	 * 
	 * @param bytes
	 ...
	 * @param length
	 *            the number of bytes to turn into a string
	 * @return the corresponding string
	 */
	 ...
	 *
	 * @param basicString
	 *            the basic PDF string, as offered by
	 *            {@link PDFObject#getStringValue()}
	...
	/**
	 * <p>
	 * Take a basic PDF string and determine if it is in UTF-16BE encoding by
	 * looking at the lead characters for a byte order marking (BOM). If it
	 ...
	 * Unicode characters. UTF-16BE and Unicode character encoding are described
	 * in the Unicode Standard by the Unicode Consortium (see the Bibliography).
	 * Note that PDFDocEncoding does not support all Unicode characters whereas
	 * UTF-16BE does.</blockquote>
	 * </p>
	 *
	 * @param basicString
	 *            the basic PDF string, as offered by
