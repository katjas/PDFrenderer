...
public class PDFImage {
		...
		private BufferedImage readImage(ImageReader jpegReader, ImageReadParam param) throws IOException {
			if (ycckcmykDecodeMode) {
				...
				if (param != null && param.getDestination() != null) {
					...
					// desired color model
					// return new BufferedImage(cm, jpegReader.read(0,
					// param).getRaster(), true, null);
	 ...
	 * 
	 * @throws IOException
	 *             if {@link PDFDecoder} throws one while evaluating if the
	 *             image is a Jpeg
	 */
	...
	private ColorModel getColorModel() {
		...
		if (cs instanceof IndexedColor) {
			...
			// If the image is a JPEG, then CMYK color space has been converted
			// to RGB in DCTDecode
